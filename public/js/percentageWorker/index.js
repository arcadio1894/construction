$(document).ready(function () {
            ...
            { data: null,
                ...
                "render": function (item)
                {
                    ...
                    if ( $.inArray('update_percentageWorker', $permissions) !== -1 ) {
                        text = text + '<a href="'+document.location.origin+ '/dashboard/editar/porcentaje/recursos/humanos/'+item.id+
                            '" class="btn btn-outline-warning btn-sm"><i class="fa fa-pen"></i></a>';
                    }
                    if ( !found){
                        if ( $.inArray('destroy_percentageWorker', $permissions) !== -1 ) {
    ...
    $formDelete.on('submit', destroyPercentageWorker);
...
function openModalDelete() {
    var percentage_id = $(this).data('delete');
    var name = $(this).data('name');

    $modalDelete.find('[id=percentage_id]').val(percentage_id);
    $modalDelete.find('[id=name]').html(name);

    $modalDelete.modal('show');
}

function destroyPercentageWorker() {
