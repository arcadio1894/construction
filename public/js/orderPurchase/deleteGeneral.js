$(document).ready(function () {
    ...
    $('#dynamic-table')
        ...
        ajax: {
            url: "/dashboard/all/order/delete",
            dataSrc: 'data'
        },
        bAutoWidth: false,
        "aoColumns": [
            ...
            { data: null,
                ...
                "render": function (item)
                {
                    ...
                    if ( $.inArray('list_orderPurchaseNormal', $permissions) !== -1 ) {
                        text = text + '<a target="_blank" href="'+document.location.origin+ '/dashboard/imprimir/orden/compra/eliminada/'+item.id+'" class="btn btn-outline-dark btn-sm" data-toggle="tooltip" data-placement="top" title="Imprimir Orden"><i class="fa fa-print"></i> </a>  ';
                    }

                    if ( $.inArray('list_orderPurchaseNormal', $permissions) !== -1 ) {
                        text = text + '<a href="'+document.location.origin+ '/dashboard/ver/orden/compra/eliminada/'+item.id+
    ...
    $.confirm({
        ...
        buttons: {
            confirm: {
                ...
                action: function (e) {
                    $.ajax({
                        url: '/dashboard/restore/order/purchase/delete/'+order_id,
                        method: 'POST',
                        ...
                        success: function (data) {
                            console.log(data);
                            $.alert("Orden de compra normal restaurada.");
                            setTimeout( function () {
                                location.reload();
                            }, 2000 )
                        },
