...
$(document).ready(function () {
    ...
    $('#material_search')
        ...
        ajax: {
            url: window.routeSearchMaterials, // tu endpoint /subequipo/materials/search
            dataType: 'json',
            delay: 250,
            ...
            processResults: function (data) {
                return {
                    results: (data || []).map(function (item) {
                        return {
                            id: item.id, // ✅ este es el ID real
                            text: item.full_name + (item.unit ? ' (' + item.unit + ')' : ''),
                            raw: item // ✅ guardamos todo el objeto para usar luego
                        };
                    })
                };
            },
        ...
        .on('select2:select', function (e) {
            ...
            let material = e.params.data.raw;
            $material = material;

            // ✅ contenedor fijo, no uses next().next().next()
            ...
            // opcional: limpiar select2 después de elegir
